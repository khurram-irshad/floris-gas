...
import "../app/find-location/results/page.css"
...
function ResultsContent() {
    ...
    return () => {
      ...
      if (searchCircle) {
        ...
        if (searchCircle.radarRings) {
          searchCircle.radarRings.forEach((ring: GoogleMapsCircle) =>
            ring.setMap(null)
          )
        }
      }
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          console.warn(
            "Geolocation not available, using search-based location only"
          )
  ...
  const initializeMapWithQuery = useCallback(
    (query: string) => {
      if (!mapRef.current) {
      ...
      // If we have user location and no search query, show nearby stations
        ...
        const userInfoWindow = new window.google.maps.InfoWindow({
          ...
          <div style="padding: 10px; text-align: center;">
            ...
            <p style="margin: 0; color: #64748b; font-size: 14px;">Lat: ${userLocation.lat.toFixed(
              6
  ...
  const searchAndShowResults = useCallback(
    (mapInstance: any, queryOverride?: string) => {
  ...
  useEffect(() => {
    if (
      ...
      autocomplete.addListener("place_changed", () => {
        ...
        const newUrl = `/find-location/results?q=${encodeURIComponent(
          trimmedQuery
        )}`
  ...
  const createSearchRadiusCircle = useCallback(
    (
      ...
      mapInstance: GoogleMapsMap,
      hasStations: boolean = true
    ) => {
  ...
  const findAndDisplayNearestStations = useCallback(
    (searchLoc: GoogleMapsLatLng, mapInstance: GoogleMapsMap) => {
      // Update loading message
      setLoadingMessage("Calculating distances to stations...")

      // Use memoized stations with distance calculations
      const currentStationsWithDistance = stationsWithDistance.map(
        (station: GasStation) => {
          const stationLocation = new window.google.maps.LatLng(
            station.lat,
            station.lng
      ...
      currentStationsWithDistance.sort(
        (a, b) => (a.distance || 0) - (b.distance || 0)
      )

      // Filter to show stations within search radius
      const nearbyStations = currentStationsWithDistance.filter(
        (station) => (station.distance || 0) <= SEARCH_RADIUS_MILES
      )
    ...
    (station: GasStation) => {
      if (map && markers[nearestStations.indexOf(station)]) {
        ...
        window.google.maps.event.trigger(
          markers[nearestStations.indexOf(station)],
          "click"
        )
      ...
      <div className="results-page">
        ...
        <main className="results-main">
            ...
            <div className="search-header">
              ...
              <div className="search-info">
                ...
                <span className="results-count">
                  {nearestStations.length} Location
                  {nearestStations.length !== 1 ? "s" : ""} Shown
                </span>
              </div>
            </div>
              ...
              {isLoading ? (
                  ...
                  <div className="error-content">
                    ...
                    <div className="error-suggestions">
                      ...
                      <ul>
                        ...
                        <li>
                          Try &quot;City, State&quot; format (e.g., &quot;Miami,
                          FL&quot;)
                        </li>
                      </ul>
                    </div>
                ...
                <div className="no-stations-found">
                  <div className="no-stations-content">
                    ...
                    <p>
                      We couldn&apos;t find any FlorisGas stations near this
                      location.
                    </p>
                  </div>
                </div>
          ...
          <div
            ...
            } ${nearestStations.length > 0 ? "has-results" : ""}`}
          ...
          >
            ...
            >
              <div className="bottom-sheet-title">
                <h2>
                  {nearestStations.length} Location
                  {nearestStations.length !== 1 ? "s" : ""} Found
                </h2>
                {nearestStations.length > 0 &&
                  nearestStations.every(
                    (station) => (station.distance || 0) <= SEARCH_RADIUS_MILES
                  ) && (
                    <span className="mobile-radius-badge">
                      Within 10 miles radius
                    </span>
                  )}
              </div>
                  ...
                  {nearestStations.map((station) => (
                    ...
                    >
                      ...
                      <div className="mobile-station-details">
                        ...
                        <p className="mobile-station-address">
                          {station.address}
                        </p>
                        <p className="mobile-station-distance">
                        ...
                        <div className="mobile-station-services">
                          ...
                          {station.services.length > 2 && (
                            <span className="mobile-service-more">
                              +{station.services.length - 2} more
                            </span>
                          )}
                        </div>
