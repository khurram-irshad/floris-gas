...
import "../app/find-location/page.css"
...
export default function FindLocationPage() {
  ...
  useEffect(() => {
    ...
    if (cachedLocation) {
      try {
        ...
        console.warn(
          "Could not parse cached location, will request fresh location"
        )
      }
    }
  ...
  useEffect(() => {
    if (
      ...
      !isGeocodingLocation &&
      !hasGeocodedLocationRef.current &&
      !searchQuery
    ) {
        ...
        <div style="padding: 10px; max-width: 300px;">
          ...
          <div style="margin-bottom: 12px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 4px; font-size: 13px;">
              ...
              <span style="color: #416AFC; font-weight: 600;">Propane: $${
                station.propanePrice
              }</span>
            </div>
          </div>
  ...
  const handleUseCurrentLocation = () => {
    if (userLocation) {
      ...
      if (window.google) {
        ...
        geocoder.geocode({ location: latLng }, (results: any, status: any) => {
          if (status === "OK" && results[0]) {
            const address = results[0].formatted_address
            router.push(
              `/find-location/results?q=${encodeURIComponent(address)}`
            )
          } else {
            // Fallback to coordinates
            router.push(
              `/find-location/results?q=${userLocation.lat},${userLocation.lng}`
            )
          }
        })
      } else {
      ...
      alert(
        "Current location not available. Please allow location access or search for a specific location."
      )
    }
  }
